...
syms W W1 W2 X Y Z X1 Y1 X2 Y2 Z1 Z2 Ai Ak Re Im;
syms ci si ck sk Rj;
equPaul=cell(8,1);
...
chgtVarRjAi=[  Rj,Ai;Rj,sym(pi)/2-Ai];
...
if test
    % equation a analyser
    syms q1 q2 q3 d1 d4 d3 a3 a2 t14 t24 t34;
    vars_xy=[q2+q3;q2]
    equs_xy=[d4*cos(q2 + q3) - a3*sin(q2 + q3) - a2*sin(q2) - t14*cos(q1) - t24*sin(q1)
        d1 - t34 + a3*cos(q2 + q3) + d4*sin(q2 + q3) + a2*cos(q2)];
    equs_1=[(cos(q3)*(a3+2)-d1) ; (-sin(q3)*(a3+2) +d4)];vars_1=q3;
    equs_4=[(sin(q3)*(a3+2)*d1+a2) ; (-cos(q3)*(d1) +d3)];vars_4=[d1,q3];

    equs_5=[(sin(q3)*(a3+2)-d1*d4+a2) ; (-cos(q3)*(d4) +d1*a2+d3)];vars_5=[d1,q3];

    vars_z=q1;equs_z=t24*cos(q1) - t14*sin(q1)
    %equs=equs_xy;vars=vars_xy;fileName='Equ1O4xy';
    %equs=equs_z;vars=vars_z;fileName='Equ1O4z';
    %equs=equs_1;vars=vars_1;fileName='Equtype1';
    %equs=equs_5;vars=vars_5;fileName='Equtype5';
    equs=equs_4;vars=vars_4;fileName='Equtype4';
end % if test==1
                ...
                switch(i_type)
                    case 0
                        ...
                        if (length(styp0)>0)
                            ...
                            styp1.Ai=subs(chgtVar(i,1),varsEqu(:),vars(:));
                            equation=styp1;
                            equation.Re=Re;
                            equation.Im=Im;
                            ...
                            s=solve({varsEqu(1)==equs_vars(1)},[v1]);
                            var1=s;
                    ...
                    case 3
                        styp3=solve(err==[0;0],[X1,Y1,X2,Y2]);
                        if (length(styp3.X1)>0)
                    ...
                    case 4
                        styp4=solve(err==[0;0],[X1,X2]);
                        ok4=length(styp4.X1)>0;
                        if ok4
                        ...
                        if (ok4)
                            ...
                            s=solve({varsEqu(1)==equs_vars(1),varsEqu(2)==equs_vars(2)},[v1,v2]);
                            var1=s.v1;var2=s.v2;
                    ...
                    case 5
                        styp5=solve(err==[0;0],[X1,Z1,X2,Z2]);
                        ok5=length(styp5.X1)>0;
                        if (ok5)
                            ok5=(diff(styp5.Z1,varsEqu(1))==0)&(diff(styp5.Z2,varsEqu(1))==0);
                        end    
                        if ok5
                            ok5=(diff(styp5.Z1,varsEqu(2))==0)&(diff(styp5.Z2,varsEqu(2))==0);
                        end
                        if (ok5)
                            equ_i_permut=permutsEqu{iPermut}*equs_i;
                            X1=styp5.X1;
                    ...
                    case 7
                        styp7=solve(err==[0;0],[W1,W2,X,Y]);
                        if (length(styp7.W1)>0)
                            equ_i_permut=permutsEqu{iPermut}*equs_i;
